...
QUERY="Applications[*].[ApplicationName,Description,DateCreated,ApplicationArn]"
FIELDS="Nome\tDescrição\tData criação\tARN"

for region in $regions; do
    echo "Buscando recursos em $region"

    instances=$(aws elasticbeanstalk describe-applications --region=$region --output text --query "$QUERY")
    ...
    if [ $? == 0 ] && [[ ! -z $instances ]]; then
        ...
        if [ ! -z "$instances" ]; then echo $instances; fi
